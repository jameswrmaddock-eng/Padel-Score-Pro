// CommonJS wrapper around the scoring engine for Jest tests.
// Mirrors src/engine/scoring.js exactly — keep in sync.

const POINTS_LABEL = ['0', '15', '30', '40'];

function createMatch(maxSets, deuceMode, tiebreakMode, playonCap) {
  return {
    maxSets,
    deuceMode,
    tiebreakMode,
    playonCap: playonCap || null,
    setsWon: [0, 0],
    setScores: [],
...
function scorePoint(state, team) {
...
function scoreRegular(s, t, opp) {
  const pts = s.points;
  if (s.isGoldenPoint) { winGame(s, t); return; }
  if (s.isDeuce) {
    if (pts[t] === 4) {
      winGame(s, t);
    } else {
...
function winGame(s, t) {
  ...
  if (!s.isTiebreak) { s.serving = s.serving === 0 ? 1 : 0; }
  s.isTiebreak = false;
  s.tbServeCount = 0;
...
function checkSetWin(s, t) {
  ...
  if (g1 === 6 && g2 === 6) {
    if (s.tiebreakMode === 'tiebreak') {
      s.isTiebreak = true; s.points = [0, 0];
      s.tbServeCount = 0; s.tbFirstServer = s.serving;
    }
    return;
  }
...
function getPointsLabel(state, team) {
...
module.exports = {
  createMatch, scorePoint, toggleServe, getPointsLabel, getStatusLabel,
};
